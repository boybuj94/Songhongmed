			...
			if($_FILES['img']['name'] != NULL){ // Đã chọn img
				...
				// Tiến hành code upload  
					if($_FILES['img']['size'] > 2448576){
						...
						$sql = "INSERT INTO danhmuc (tendm,ngonngu,img,info,link_title) VALUES ('{$ten}','{$nn}','{$name}','{$info}','{$str}')";
			...
			if($_FILES['img']['name'] != NULL){ 
				...
				// Tiến hành code upload  
					if($_FILES['img']['size'] > 2548576){
						...
						// img hợp lệ, tiến hành upload
						move_uploaded_file($_FILES['img']['tmp_name'], '../public/img/anh/'.$name);
						$sql = "UPDATE danhmuc SET tendm = '{$ten}',info = '{$info}', ngonngu='{$nn}',img='{$name}',link_title='{$str}' WHERE iddm = '{$id}'";
							if(mysqli_query($con,$sql)){
								echo "<script>";
				...
				$sql = "UPDATE danhmuc SET tendm = '{$ten}',info = '{$info}',ngonngu='{$nn}',link_title='{$str}' WHERE iddm = '{$id}'";
							if(mysqli_query($con,$sql)){
								echo "<script>";
