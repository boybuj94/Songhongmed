...
<div class='duongdan'>
	<p><a href='index.php'>Trang chủ &nbsp;&nbsp;</a>&rsaquo;<a href='index.php?ql=nt&ac=v'>&nbsp;&nbsp;Nội thất y tế</a></p>
</div>
<div class='admin'>
...
if($_SESSION['quyen'] == '0' || $_SESSION['quyen'] == '2'){
	if(isset($_REQUEST['ac']) || isset($_REQUEST['search'])){
    	...
    	<thead>
    		<caption><p>DANH SÁCH SẢN PHẨM NỘI THẤT Y TẾ</NAV></p></caption>
            <tr style='text-align:center; background:#0099ff;height:45px;color:#fff; font-weight:bold; '>
				...
				<!-- <select name='ajax' class="loaisp">
					<option value="">Menu Nội Thất</option>
					<?php 
					$sql3 = "SELECT tennt,idnt FROM noithat WHERE idnt in (SELECT sanpham_nt.idnt FROM noithat,sanpham_nt WHERE noithat.idnt=sanpham_nt.idnt)";
					$sanpham_nt3 = mysqli_query($con,$sql3);
					while($rows3 = mysqli_fetch_array($sanpham_nt3)){
						echo "<option value='{$rows3[idnt]}'>{$rows3[tennt]}</option>";	
					} ?>
				</select> -->
				</td>
				<td>Độc Quyền</td>				
	...
	?> 
		...
		<tbody class='list'>
            ...
            ?><tr>
                ...
                <td width='40px'>
					<a href='index.php?ql=nt&ac=sua&id=<?php echo $rows['idsp']?>' title='Sửa thông tin'>
						<div class='admin-icon' style='background-image:url(public/img/icon/icon-sua.png);'> </div>
					</a>
				</td>
                <td width='40px'>
					<a onClick='javascript: return CheckSure();' href='index.php?ql=nt&ac=xoa&id=<?php echo $rows['idsp']?>' title='Xóa admin'>
            ...
            echo "<tr height='50px;'>
            	<td colspan='12'>
                	<div class='admin-them'>
                    	<div class='admin-add'><a href='index.php?ql=nt&ac=add' title=''>&nbsp;&nbsp;+ Thêm mới&nbsp;&nbsp;</a></div>
                        <div class='admin-tt'><p>Hiển thị từ 1 đến {$num} / Tổng {$num}</p></div>
                        <div class='admin-phantrang'>";
                        	if($_GET['page']>1){
								$page =$_GET['page']-1;
								  
								echo "<a href='index.php?ql=nt&ac=v&page={$page}'>Back </a>";
							  ...
							  for ( $j = 1; $j <= $sotrang; $j ++ ){
								  if($j == $_GET['page']){
									  echo $j;
										  ...
										  echo "<a href='index.php?ql=nt&ac=v&page={$j}'> {$j} </a>";	
									  }
									  }
									  if($_GET['page']<$sotrang){
										$page =$_GET['page']+1;	
									  echo "<a href='index.php?ql=nt&ac=v&page={$page}'> Next</a>";;
									  }
                        echo "</div>
		...
		error_reporting(0);	
			...
			?>
			<form method='post' action='model/m_sanpham_nt.php' enctype='multipart/form-data' name='dangky'>
				
				<table border='0'>
				<caption><p>THÊM MỚI SẢN PHẨM NỘI THẤT Y TẾ</p></caption>
					<tr>
					...
					<tr>
                        ...
                        <select name = 'dm'>
							<?php
								$loai2 = 'SELECT idnt,tennt FROM noithat order by ngonngu asc';
								$ketqua = mysqli_query($con,$loai2);
								while($dong = mysqli_fetch_array($ketqua)){
			 				?>
								<option value="<?php echo $dong['idnt']; ?>" >
			...
			?>
			<form method='post' action='model/m_sanpham_nt.php?id=<?php echo $id; ?>' enctype='multipart/form-data' name="update">			
				<table border='0'>
				<caption><p>SỬA SẢN PHẨM NỘI THẤT Y TẾ</p></caption>
					<tr>
						<td  width = '120px'><label for='ten'>Tên sản phẩm: </label></td>
					...
					<tr>
						...
						<td>
                            ...
                            <a onClick='javascript: return CheckSure();' href='index.php?ql=nt&ac=v' title='Hủy thao tác'>
...
<?php
			...
			<table border='0' style="padding:10px">
				<caption><p>CHI TIẾT SẢN PHẨM NỘI THẤT Y TẾ</p></caption>
					<tr>
					...
					<tr>
						...
						<td>
                            ...
                            <a onClick='javascript: return CheckSure();' href='index.php?ql=nt&ac=sua&id=<?php echo $id; ?>' title='Sửa sản phẩm'>
