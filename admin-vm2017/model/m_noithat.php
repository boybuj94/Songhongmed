<?php
	...
	if (isset($_REQUEST['add'])){ //    ----------------THÊM DỮ LIỆU ----------------
			...
			if($_FILES['img']['name'] != NULL){ // Đã chọn img
				...
				// Tiến hành code upload  
					if($_FILES['img']['size'] > 2448576){
						...
						$sql = "INSERT INTO noithat (tennt,ngonngu,img,link_title) VALUES ('{$ten}','{$nn}','{$name}','{$str}')";
							if(mysqli_query($con,$sql)){
								echo "<script>";
								echo "alert('Thêm thành công menu nội thất!');";
								echo "window.location.href='../index.php?ql=nnt&ac=v';";
								echo "exit();";
								echo "</script>";
								...
								echo "window.location.href='../index.php?ql=nnt&ac=add';";
								echo "exit();";
								echo "</script>";
							}
					}
			...
			if($_FILES['img']['name'] != NULL){ 
				...
				// Tiến hành code upload  
					if($_FILES['img']['size'] > 2548576){
						...
						// img hợp lệ, tiến hành upload
						move_uploaded_file($_FILES['img']['tmp_name'], '../public/img/anh/'.$name);
						$sql = "UPDATE noithat SET tennt = '{$ten}',ngonngu='{$nn}',img='{$name}',link_title='{$str}' WHERE idnt = '{$id}'";
							if(mysqli_query($con,$sql)){
								echo "<script>";
								...
								echo "window.location.href='../index.php?ql=nnt&ac=sua&id={$id}'";
								echo "exit();";
								echo "</script>";
							}
					}
				...
				$sql = "UPDATE noithat SET tennt = '{$ten}', ngonngu ='{$nn}', link_title = '{$str}' WHERE idnt = '{$id}'";
							if(mysqli_query($con,$sql)){
								echo "<script>";
		...
		$id = $_REQUEST['id'];
		$sql = "DELETE FROM noithat WHERE idnt = '{$id}'";
		if(mysqli_query($con,$sql)){
			echo "<script>";
