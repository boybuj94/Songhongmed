		...
		$date = date('YmdHis-');// lấy thời gian thực 
			if($_FILES['img']['name'] != NULL){ // Đã chọn img
				...
				// Tiến hành code upload  
					if($_FILES['img']['size'] > 327680){
						echo "img không được lớn hơn 4mb";
					}else{
						// img hợp lệ, tiến hành upload

						move_uploaded_file($_FILES['img']['tmp_name'], '../public/img/anh/'.$name);
						$sql = "INSERT INTO sanpham (tensp,masp,iddm,hangsp,thongtin,baiviet,ngonngu,img,noibat,link_title) VALUES ('{$ten}','{$ma}','{$dm}','{$hang}','{$thongtin}','{$bv}','{$nn}','{$name}','{$nb}','{$str}')";
							if(mysqli_query($con,$sql)){
								echo "<script>";
								echo "alert('Thêm thành công sản phẩm!');";
								echo "window.location.href='../index.php?ql=sp&ac=v';";
								echo "exit();";
								echo "</script>";	
							}else{
								echo "<script>";
								echo "alert('Đã xảy ra lỗi');";
								echo "window.location.href='../index.php?ql=sp&ac=add';";
								echo "exit();";
								echo "</script>";
			...
			if($_FILES['img']['name'] != NULL){ 
				...
				// Tiến hành code upload  
					if($_FILES['img']['size'] > 327680){
						...
						move_uploaded_file($_FILES['img']['tmp_name'], '../public/img/anh/'.$name);
						$sql = "UPDATE sanpham SET tensp = '{$ten}',masp = '{$ma}',iddm = '{$dm}',hangsp = '{$hang}',thongtin = '{$thongtin}',baiviet = '{$bv}',ngonngu='{$nn}',img='{$name}',noibat ='{$nb}',link_title ='{$str}' WHERE idsp = '{$id}'";
							if(mysqli_query($con,$sql)){
								echo "<script>";
								...
								echo "window.location.href='../index.php?ql=sp&ac=sua&id={$id}&id={$id}'";
								echo "exit();";
								echo "</script>";
							}
					}
					...
					echo "window.location.href='../index.php?ql=sp&ac=sua&id={$id}';";
					echo "exit();";
					echo "</script>";
				}
		   }else if($_FILES['img']['name'] == NULL){ // trường hợp img NULL
				$sql = "UPDATE sanpham SET tensp = '{$ten}',masp = '{$ma}',iddm = '{$dm}',hangsp = '{$hang}',thongtin = '{$thongtin}',baiviet = '{$bv}',ngonngu='{$nn}',noibat ='{$nb}',link_title ='{$str}' WHERE idsp = '{$id}'";
							if(mysqli_query($con,$sql)){
								echo "<script>";
